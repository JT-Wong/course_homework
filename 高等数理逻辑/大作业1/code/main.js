...
function cal_one_num(n){
    ...
    while(n !== 0){
        ...
        n = n - Math.pow(2, parseInt(Math.log(n) / Math.log(2)));
        res += 1;
    }
    return res;
}
...
function pro_logic_syntax_check(pro_logic, custom_logic_list, zero_ele_custom_logic_list) {
    ...
    for (i = 0; i < pro_logic.length; i++){
        if (pro_logic[i] === '→' || pro_logic[i] === '↔'){
            if (temp === ''){
                result = [false, pro_logic[i] + " left cnt't be null", []];
                error_print('', 0, 0, pro_logic[i] + " left cnt't be null");
                return result;
            }
            else {
                temp = '';
    ...
    if (temp === '') {
        result = [false, pro_logic[i-1] + " right cnt't be null", []];
        error_print('', 0, 0, pro_logic[i-1] + " right cnt't be null");
        return result;
    }
    ...
    */
    ...
    if (result[0] === true ){
        if (bracket_stack.length !== 0){
            result = [false, 'lost )', []]
            error_print('', 0, 0, 'lost )');
        }

        if (!(state === 0 || state === 1 || state === 2 )){
            result = [false, 'proposition not complete', []];
            error_print('', 0, 0, 'proposition not complete');
        }
    }
...
function custom_logic_calculate(custom_logic_word, pro_logic, custom_logic_list, zero_ele_custom_logic_list, true_table, true_table_id, begin_index){
    ...
    if (pro_logic[i] === '('){
        ...
        error_print('', 0, 0, 'lost ( after ' + custom_logic_word);
        throw SyntaxError();
    }
    ...
    if (element_num !== parseInt(custom_logic_list[custom_logic_word][0])){
        ...
        error_print('', 0, 0, custom_logic_word + ' element num is not right');
        throw SyntaxError();
    }
        ...
        while (i < formula.length){
            if (formula[i] === basic_logic_word){
                ...
                while (j < formula.length){
                    ...
                    // 遇到优先级更高的联结词
                    else if (bracket_stack ===0 && (basic_logic_list.indexOf(formula[j])!==-1) && (basic_logic_list.indexOf(formula[j]) > k)){
                        formula = formula.substring(0,j) + ')' + formula.substring(j);
                        break;
                    }
                    j += 1;
    ...
    for (i = 0; i < line_list.length; i++){
        ...
        if (line_str[0] === '#'){
            ...
            if (custom_logic_name in custom_logic_list){
                // window.alert('repeat define custom logic ' + custom_logic_name);
                error_print('', 0, 0, 'repeat define custom logic ' + custom_logic_name);
                return;
            }
            else if (Math.pow(2, parseInt(element_num)) !== custom_true_table.length){
                // window.alert(custom_logic_name + ' define wrong');
                error_print('', 0, 0, custom_logic_name + ' define wrong');
                return;
            }
